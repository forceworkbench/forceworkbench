...
require_once('header.php');
		...
		foreach($config as $configKey => $configValue){
			if($configValue['overrideable']){
				...
				if($configValue['dataType'] == "boolean"){
					...
					print "<input name='$configKey' id='$configKey' type='text' value='". $_SESSION[config][$configKey] . "' size='10'/></td>\n";
				} else {
					print "</td>\n";
				}
				print "\t</tr>\n";
	...
	print "<tr> <td colspan='3' align='right'><input type='submit' name='submitConfigSetter' value='Apply Settings'/>&nbsp;<input type='reset' value='Cancel'/></td> </tr>";
