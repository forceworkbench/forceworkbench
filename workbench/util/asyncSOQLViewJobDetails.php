...
include "$root/soxl/QueryObjects.php";
include "$root/async/QueryFutureTask.php";
include "$root/session.php";
include "$root/shared.php";

// // for REST calls
include "$root/controllers/RestExplorerController.php";
include "$root/async/RestExplorerFutureTask.php";
...
function displayJobDetails($jobId) {
    $c = getRestExplorerInstanceForAsyncSOQL($jobId,'GET');
...
function printResult($viewJobsResult,$jobId) {
    ...
    if ($jobIdValue != null && $statusComplete == true) {
        print "<a id='viewResultsToggler' href='javascript:toggleTable();'>Show Target Object SOQL results</a>";
...
function cancelJob($jobId) {
    $c = getRestExplorerInstanceForAsyncSOQL($jobId,'DELETE');
    $f = new RestExplorerFutureTask($c);
    $f->returnUnformattedResult(true);
...
function generateSoql($viewJobsResultInst, $jobIdValue, $jobId){
    ...
    //SOQL inserts only first 15 charaters in target value. Trimming the jobId string does not affect the query as the first 15 characters of jobId are unique for every job.
    $trimmedJobId = substr($jobId, 0, 15); 
    $query = $query." FROM ".$targetObject." WHERE ".$jobIdValue." = '".$trimmedJobId."'";
